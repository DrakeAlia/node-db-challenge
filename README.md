...
-Relational databases is used to save data and SQL is a language that is used to comunicate with RDMS.

- [X] Why do tables need a `primary key`?

-Tables need a primary key because it guarantees a unique identifier for each entry. If we were to use name or another field as the main identifier, we run the risk of having multiple entries with the same key.
...
-To create a many to many relationship, we add an intermediary table made up of foreign keys designed to reference primary keys on each of the other tables.
